...
class PaginaOperacionesMatriciales(PaginaBase):
    ...
    def configurar_grid(self):
        # Ajustamos filas: 0 para nav, 1 selector, 2 matrices, 3 controles, 4 resultados
        self.grid_rowconfigure(0, weight=0) # Navegación
        self.grid_rowconfigure(1, weight=0) # Selector
        self.grid_rowconfigure(2, weight=1) # Matrices (Expandible)
        self.grid_rowconfigure(3, weight=0) # Controles
        self.grid_rowconfigure(4, weight=1) # Resultados
        self.grid_columnconfigure(0, weight=1)

    def crear_barra_navegacion(self):
        """Crea una barra superior para navegar entre las herramientas de Álgebra."""
        ...
        # Botón activo (Deshabilitado visualmente o con color diferente)
        ...
        try:
            ...
            for i in range(filas_a):
                for j in range(cols_a):
                    ...
                    # e.insert(0, f"{i+1}{j+1}") # ELIMINADO: Ahora inicia vacío
            for i in range(filas_b):
                for j in range(cols_b):
                    ...
                    # e2.insert(0, f"{i+1}{j+1}") # ELIMINADO: Ahora inicia vacío
            self.limpiar_operaciones(borrar_entradas=False) # No borrar lo que acabamos de crear
    ...
    def leer_entradas_cuadricula(self, entradas, opcional=False):
        """
        Lee los datos de la cuadrícula.
        Si opcional=True y la cuadrícula está vacía, retorna None en lugar de error.
        """
        ...
        if celdas_no_vacias == 0:
            if opcional: return None
            if filas * cols > 0: raise ValueError("La matriz no puede estar vacía")
    ...
    def calcular_operacion_matricial(self):
        ...
        try:
            # Ahora la lectura es opcional para permitir operaciones individuales
            A = self.leer_entradas_cuadricula(self.entradas_a, opcional=True)
            B = self.leer_entradas_cuadricula(self.entradas_b, opcional=True)
            
            operacion = self.operacion_matricial_var.get()
            R = []

            # Mostrar matrices de entrada que existen
            texto_entrada = ""
            if A: texto_entrada += f"Matriz A ({len(A)}x{len(A[0])}):\n{self._matriz_a_pretty(A)}\n\n"
            if B: texto_entrada += f"Matriz B ({len(B)}x{len(B[0])}):\n{self._matriz_a_pretty(B)}"
            
            if texto_entrada:
                pasos.append({'titulo': "Matrices de Entrada:", 'math': texto_entrada})
            else:
                raise ValueError("Debe ingresar datos en al menos una matriz.")
            ...
            if operacion in ('Suma','Resta'):
                ...
                # Caso 1: Solo A
                if A and not B:
                    R = [[alpha*A[i][j] for j in range(len(A[0]))] for i in range(len(A))]
                    pasos.append({'titulo': f"Operación: α·A (α={fmt(alpha)})", 'math': self._matriz_a_pretty(R)})
                    
                # Caso 2: Solo B
                elif B and not A:
                    # Aplicamos el signo de la operación (ej: Resta -> -1 * beta * B)
                    factor = sgn * beta
                    R = [[factor*B[i][j] for j in range(len(B[0]))] for i in range(len(B))]
                    titulo_op = f"Operación: {('+' if sgn>0 else '-')} β·B (β={fmt(beta)})"
                    pasos.append({'titulo': titulo_op, 'math': self._matriz_a_pretty(R)})
                    
                # Caso 3: A y B (Operación completa)
                else:
                    fa, ca = len(A), len(A[0])
                    fb, cb = len(B), len(B[0])
                    if fa != fb or ca != cb: 
                        raise ValueError(f"Para {operacion.lower()}: dimensiones incompatibles A({fa}x{ca}) != B({fb}x{cb})")
                    ...
                    pasos.append({'titulo': f"Resultado: (α·A) {op_str} (β·B)", 'math': self._matriz_a_pretty(R)})

            elif operacion == 'Multiplicacion':
                if not A or not B: raise ValueError("Para multiplicación se requieren ambas matrices A y B.")
                fa, ca = len(A), len(A[0])
                fb, cb = len(B), len(B[0])
                
                if ca != fb: raise ValueError(f"Para multiplicación: cols(A)={ca} != filas(B)={fb}")
