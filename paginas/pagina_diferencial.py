...
from app_config import COLOR_DIFERENCIAL, COLOR_FONDO_SECUNDARIO
...
class PaginaDiferencial(PaginaBase):
    def crear_widgets(self):
        ...
        ctk.CTkRadioButton(marco_metodo, text="Derivada", state="disabled").grid(row=0, column=1, padx=8)
        ctk.CTkRadioButton(marco_metodo, text="Límite", state="disabled").grid(row=0, column=2, padx=8)
        ...
        ctk.CTkLabel(marco_entradas, text="f(x) =", font=ctk.CTkFont(size=13)).grid(row=0, column=0, sticky="w", padx=8, pady=6)
        ctk.CTkEntry(marco_entradas, placeholder_text="Ej: sin(x) * x^2", state="disabled").grid(row=0, column=1, sticky="ew", padx=8, pady=6)
        ...
        lbl_info = ctk.CTkLabel(self.pasos_scroll_frame, 
                                text="🚧 En construcción... 🚧\n\nAquí irán las herramientas de límites y derivadas.",
