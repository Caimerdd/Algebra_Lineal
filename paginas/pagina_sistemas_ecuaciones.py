...
# Importación de Complement con manejo de errores
try:
    ...
    # Funciones dummy para no romper la UI si falla el import
...
class PaginaSistemasEcuaciones(PaginaBase):
    ...
    def configurar_grid(self):
        # Row 0: Nav, Row 1: Selector, Row 2: Ecuaciones, Row 3: Matrices, Row 4: Controles, Row 5: Resultados
        self.grid_rowconfigure(0, weight=0)
        self.grid_rowconfigure(1, weight=0)
        self.grid_rowconfigure(2, weight=0)
        self.grid_rowconfigure(3, weight=0) # Antes 1
    ...
    def crear_interfaz(self):
        ...
        ctk.CTkLabel(marco_ecuaciones, text="Ingrese el sistema (ej: 2x + y = 10):", 
        ...
        # INICIO LIMPIO: No insertamos texto de ejemplo
        ...
        # Marco para matrices (Visualización Grid)
        marco_matrices = ctk.CTkFrame(self)
        marco_matrices.grid(row=3, column=0, sticky="nsew", padx=12, pady=8)
        ...
        ctk.CTkLabel(header_b, text="Vector de Términos (B)", font=ctk.CTkFont(size=14, weight="bold")).grid(row=0, column=0, sticky="w")
        ...
        # Columna de B fija en 1 para sistemas de ecuaciones
        ...
        ctk.CTkButton(marco_controles, text="Generar Cuadrículas (Manual)", command=self.generar_cuadriculas_matriz, fg_color=COLOR_ALGEBRA, hover_color=COLOR_HOVER).grid(row=0, column=0, padx=6)
    ...
    def _matriz_a_pretty(self, M):
        """Formatea una matriz usando SymPy (si disponible) o texto simple."""
        try:
    ...
    def _renderizar_pasos_gauss(self, pasos_lista: list, ops_lista: list):
        """Dibuja los pasos de Gauss/Inversa."""
        ...
        # ops_lista podría tener un elemento extra al inicio ('Matriz Inicial'), ajustamos
        ops_a_mostrar = ops_lista[:len(pasos_a_mostrar)]
        ...
        if len(pasos_lista) > MAX_PASOS_VISUALES:
            self._crear_bloque_texto("...", f"({len(pasos_lista) - MAX_PASOS_VISUALES} pasos más omitidos para mejorar rendimiento)")
    ...
    def _renderizar_pasos_cramer(self, pasos_lista: list):
        """Dibuja los pasos de Cramer."""
        self._limpiar_pasos_scroll()
        titulo_actual = "INICIO"
        math_actual = []
        
        for linea in pasos_lista:
            linea = str(linea).strip() # Asegurar que es string
            if not linea: continue
            
            # Detectamos si es un título nuevo (generalmente termina en :)
            if not linea.startswith(" ") and ":" in linea and not "x" in linea[:4]:
                if math_actual:
    ...
    def generar_cuadriculas_matriz(self):
        try:
            ...
            # Crear nuevas entradas para matriz A (VacíAS)
            ...
            # Crear nuevas entradas para matriz B (VacíAS)
    ...
    def parsear_y_poblar_ecuaciones(self):
        try:
            ...
            if not lineas:
                raise ValueError("No hay ecuaciones válidas. Asegúrate de usar el signo '='.")

            var_map, var_ordenadas = {}, []
            matriz_coef_dict, vector_const = [], []
            # Regex robusto para capturar +/- coeficientes y variables
            ...
            if not var_ordenadas:
                raise ValueError("No se detectaron variables (ej: x, y, z).")
            ...
            self.resultado_caja.configure(state="normal")
            self.resultado_caja.delete('0.0','end')
            self.resultado_caja.insert('0.0', f"Sistema detectado: {num_filas} ecuaciones x {num_vars} variables.\nVariables: {', '.join(var_ordenadas)}")
            self.resultado_caja.configure(state="disabled")
                
        except ValueError as e:
            self.resultado_caja.configure(state="normal")
            self.resultado_caja.delete('0.0','end')
            self.resultado_caja.insert('0.0', f"Error al leer ecuaciones: {e}")
            self.resultado_caja.configure(state="disabled")

    def poblar_cuadricula(self, entradas_grid, datos):
        filas_grid = len(entradas_grid)
        cols_grid = len(entradas_grid[0]) if entradas_grid else 0
        filas_datos = len(datos)
        cols_datos = len(datos[0]) if datos else 0
        ...
        try:
            if metodo == 'Regla de Cramer':
                ...
                if res['estado'] == 'exito':
                    ...
                    resultado_texto = f"Solución única encontrada:\n{sol_texto}"
                else:
                    resultado_texto = f"Resultado: {res['mensaje']}"
                ...
                if status=='unique' and sol is not None:
                    resultado_texto = 'Solución única:\n' + '\n'.join(f'x{i+1} = {fmt(v)}' for i,v in enumerate(sol))
                elif status=='inconsistent': 
                    resultado_texto = 'El sistema es inconsistente (sin solución)'
                elif status=='infinite':
                    libres = res.get('free_vars', []); base = res.get('basic_solution', {})
                    # Mejorar visualización de infinitas soluciones
                    lineas = []
                    total_vars = len(A[0])
                    for i in range(total_vars):
                        if i in libres:
                            lineas.append(f'x{i+1} = variable libre')
                        else:
                            val = base.get(i, 0.0)
                            lineas.append(f'x{i+1} = {fmt(val)}  (asumiendo libres=0)')
    ...
    def limpiar_matrices(self, borrar_grid=True):
        ...
        # No re-insertamos texto, lo dejamos limpio