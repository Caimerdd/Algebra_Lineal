...
class PaginaMetodosNumericos(PaginaBase):
    ...
    def configurar_grid(self):
        # 0: Nav, 1: Selector, 2: Función, 3: Controles, 4: Resultados
    ...
    def crear_interfaz(self):
        ...
        self.ent_funcion_fx = ctk.CTkEntry(marco_funcion, placeholder_text="Ej: x^3 - x - 2  o  exp(x) - 3*x")
    ...
    def _crear_bloque_texto(self, titulo: str, texto: str):
        ...
        # Usamos fuente monospace para que el "dibujo" de la ecuación salga bien
    ...
    def _procesar_texto_funcion(self, texto):
        """Traduce 'ln' a 'log' y 'e^' a 'exp' para que SymPy entienda."""
    ...
    def calcular_operacion_numerica(self):
        ...
        try:
            ...
            # 1. Visualización "Math on Paper"
            funcion_str = self._procesar_texto_funcion(funcion_raw)
            try:
                x = sp.Symbol('x')
                expr = sp.sympify(funcion_str)
                # TRUCO VISUAL: Eliminar el punto de multiplicación (2⋅x -> 2x)
                texto_bonito = sp.pretty(expr, use_unicode=True).replace('⋅', '')
                self._crear_bloque_texto("FUNCIÓN A ANALIZAR", texto_bonito)
            except:
            ...
            if res['estado'] == 'exito':
                txt = f"Raíz Aproximada: {res['raiz']:.10f}\nIteraciones: {res['iteraciones']}\nError Final: {res.get('error', 0):.6e}"
    ...
    def graficar_funcion_interna(self):
        try:
            ...
            # Marcadores de intervalo y raíz
            try:
                if metodo != "Newton-Raphson" and a_str and b_str:
